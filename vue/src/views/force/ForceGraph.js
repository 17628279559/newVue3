...
const ForceGraph = (
  { nodes, links },
  {
    ...
    linkStroke = '#8EC5FC', // link stroke color
  ...
  const forceNode = d3.forceManyBody().strength(-280)
  const forceLink = d3
    .forceLink(links)
    .id(({ index: i }) => N[i])
    .distance(80)
  ...
  const node = svg
    ...
    .on('mouseover', (index, item) => {
      ...
      $('#force_message').text(item.id)
    })
    .on('mouseout', (index, item) => {
      d3.select(index.target).transition().duration(150).attr('r', nodeRadius)
      $('#force_message').css({ display: 'none' })
    })
    .on('mousemove', ev => {
      $('#force_message').css({ left: ev.clientX + 5 + 'px', top: ev.clientY - 40.4 + 'px' })
    })
    .call(drag(simulation))

  svg.append('defs').append('marker').attr('id', `arrow`).attr('viewBox', '0 -5 10 10').attr('refX', 15).attr('refY', -0.5).attr('markerWidth', 6).attr('markerHeight', 6).attr('orient', 'auto').append('path').attr('fill', '#FBAB7E').attr('d', 'M0,-5L10,0L0,5')
