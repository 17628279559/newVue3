...
const scale = () => {
  ...
  xScale = d3
    ...
    .range([0, innerwidth])
    .padding(0.2)
  yScale = d3.scaleLinear().domain([max, min]).range([0, innerheight])
...
const update = (response) => {
  ...
  canvas.select('#xx').transition().duration(500).call(xAxis)
...
onMounted(() => {
  Axios.get(getdataApi + `?use_fake=${proxy.$usefake ? 'true' : 'false'}`).then(
    (response) => {
      data = response.data.data
      time = response.data.time
      ...
      // 使用比例尺画y轴
      yAxisGroup = canvas.append('g').attr('id', 'yy').call(yAxis)
      // 添加y轴信息
      yAxisGroup
        .append('text')
      ...
      // 调整 x轴y轴字体大小
      d3.selectAll('.tick').attr('font-size', `${1.3 * ratio}em`)
      ...
      canvas
        ...
        .attr(
          'transform',
          `translate(${innerwidth / (1.1 * ratio)},${
            innerheight + margin.bottom / 2
          })`
        )
        .attr('text-anchor', 'middle')
...
<template>
  <div id="all">
    <svg class="center" :width="width" :height="height" ref="svgRef">
    </svg>
  </div>
